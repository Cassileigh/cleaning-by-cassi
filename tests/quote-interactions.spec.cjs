...
for (const failure of ['provider', 'malformed']) {
  test(`Quote retry after ${failure} failure`, async ({ page }) => {
    ...
    await page.locator('[name="cf-turnstile-response"]').evaluate((el) => {
      el.value = 'fresh-token';
    });
    await page.locator('#quote-submit').click();
...
test('A stalled quote request times out and restores the form', async ({
  page,
}) => {
  ...
  await page.route('**/api/quote', async (route) => {
    ...
    await route
      .fulfill({ contentType: 'application/json', body: '{"ok":true}' })
      .catch(() => {});
  });
  await prepare(page);
  await page.locator('#quote-submit').click();
...
test('Restored quote form clears a stale Sending state and preserves retry identity', async ({
  page,
}) => {
  await prepare(page);
...
test('Back after success restores a usable quote form', async ({ page }) => {
  await page.route('**/api/quote', (route) =>
